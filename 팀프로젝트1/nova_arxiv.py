...
def clean_text(text):
    ...
    # text = re.sub(r"\\emph\{.*?\}", "", text)  # \emph 제거
    ...
    # text = re.sub(r"%.*?$", "", text, flags=re.MULTILINE) # %로 시작하는 주석 제거
    # text = re.sub(r"\\begin\{.*?\}|\\end\{.*?\}", "", text)  # \begin, \end 제거
    # text = re.sub(r"\{.*?\}", "", text)  # {} 안의 내용 제거 (중괄호도 제거)
...
def extract_info(arxiv_id, download_dir="nova_arxiv_papers"):
    try:
        ...
        if source_file_path.endswith(".tar.gz"):
            ...
            # .tex 파일 경로
            # tex_file_path = os.path.join(download_dir, "main.tex")
            # tex_file_path = os.path.join(download_dir, "FTM-arxiv.tex")
        ...
        with open(tex_file_path, "r", encoding="utf-8") as f:
            ...
            # abstract_alternate_match = re.search(r"\\abstract\{(.*?)\}", content, re.DOTALL)
            # if abstract_alternate_match:
            #     abstract = abstract_alternate_match.group(1).strip()
